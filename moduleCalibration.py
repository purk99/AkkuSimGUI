...
class ModulKalibirerungAllgemein(ttk.Frame):
    def __init__(self,parent):
        ...
        ttk.Label(self,text="Shuntspannung kalibrieren:").grid(column=0,row=2)
        ...
        self.calValFineCombo = ttk.Combobox(self,values=CalibrationCoarse).grid(column=1,row=7)
        self.calValCoarseCombo = ttk.Combobox(self,values=CalibrationFine).grid(column=2,row=7)
    ...
    def calibrateMeas_Add(self):
        ...
        match switchVal:
            case 0: self.meas.setShuntOffset(CalibrationCoarse+CalibrationFine)
            case 1: self.meas.setBusVoltOffset(CalibrationCoarse+CalibrationFine)
            case 2: self.meas.setBusCurrOffset(CalibrationCoarse+CalibrationFine)
            case 3: self.meas.setBusPowerOffset(CalibrationCoarse+CalibrationFine)

    def calibrateMeas_Subtract(self):
        switchVal = self.calSelectCombo.current()
        match switchVal:
            case 0: self.meas.setShuntOffset(-1*(CalibrationCoarse+CalibrationFine))
            case 1: self.meas.setBusVoltOffset(-1*(CalibrationCoarse+CalibrationFine))
            case 2: self.meas.setBusCurrOffset(-1*(CalibrationCoarse+CalibrationFine))
            case 3: self.meas.setBusPowerOffset(-1*(CalibrationCoarse+CalibrationFine))
